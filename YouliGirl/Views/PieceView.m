...
@synthesize image, number, edges, position, angle, size, tempAngle, padding, delegate, neighbors, oldPosition, positionInDrawer, group;
	...
	if (!self.hasNeighbors) {
		...
		if ([gesture state]==UIGestureRecognizerStateEnded || [gesture state]==UIGestureRecognizerStateCancelled || [gesture state]==UIGestureRecognizerStateFailed) {
			...
			[UIView animateWithDuration:0.2 animations:^{
				...
				self.isRotating = NO;
				self->delegate.drawerView.userInteractionEnabled = YES;
				[self->delegate pieceRotated:self];
			}];
